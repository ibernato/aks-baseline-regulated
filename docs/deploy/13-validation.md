...
1. Get the public IP of Azure Application Gateway.
   ...
   ```bash
   APPGW_PUBLIC_IP=$(az deployment group show -g rg-production-networking-spokes -n spoke-production-01 --query properties.outputs.appGwPublicIpAddress.value -o tsv)
   ```
