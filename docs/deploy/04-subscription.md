...
1. Perform subscription-level deployment.
   ...
   az deployment sub create -f subscription.bicep -l australiaeast -p enableMicrosoftDefenderForCloud=false enforceAzureDefenderAutoDeployPolicies=false networkWatcherRGRegion="${NETWORK_WATCHER_RG_REGION}"
   ```
