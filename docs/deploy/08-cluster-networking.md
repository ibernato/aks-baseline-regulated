...
1. Deploy the cluster spoke.
   ...
   az deployment group create -g rg-enterprise-networking-spokes -f networking/spoke-BU0001A0005-01.bicep -p location=australiaeast hubVnetResourceId="${RESOURCEID_VNET_HUB}"
   ```
...
1. Update the regional hub deployment to account for the runtime requirements of the virtual network.
   ...
   az deployment group create -g rg-enterprise-networking-hubs -f networking/hub-region.v2.bicep -p location=australiaeast aksImageBuilderSubnetResourceId="${RESOURCEID_SUBNET_AIB}" nodepoolSubnetResourceIds="${RESOURCEID_SUBNET_NODEPOOLS}" aksJumpboxSubnetResourceId="${RESOURCEID_SUBNET_JUMPBOX}"
   ```
